...
def online_mine_all(labels, embeddings, margin, squared=False, device='cpu'):
    ...
    pos_triplet_loss_avg = torch.sum(triplet_loss) / (num_positive_triplets + 1e-16)
    all_triplet_loss_avg = torch.sum(triplet_loss) / (num_valid_triplets + 1e-16)
    return pos_triplet_loss_avg, num_positive_triplets, num_valid_triplets, all_triplet_loss_avg
...
def _get_anchor_positive_triplet_mask(labels, device):
    ...
    indices_equal = torch.eye(len(labels)).bool().to(device)
