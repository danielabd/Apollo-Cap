...
def train(model, optimizer, df_train, df_val, labels_set_dict, labels_idx_to_str, path_for_saving_last_model, path_for_saving_best_model, device, tgt_file_vec_emb, config,pos_combinations_labels, neg_combinations_labels, **kwargs):
    ...
    for epoch in range(config['epochs']):
        ...
        for step, (tokenized_texts_list, labels, texts_list) in enumerate(pbar:= tqdm(train_dataloader, desc="Training", leave=False,)): #model based on bert
            ...
            # triplet loss
            loss, num_positive_triplets, num_valid_triplets, all_triplet_loss_avg = mining.online_mine_all(labels, outputs, float(config['margin']), device=device)
...
def main():
    ...
    pos_combinations_labels = None; neg_combinations_labels = None

    train(model, optimizer, df_train, df_val, labels_set_dict, labels_idx_to_str, path_for_saving_last_model,path_for_saving_best_model, device, tgt_file_vec_emb,config,pos_combinations_labels, neg_combinations_labels)
