...
def get_args():
    ...
    parser.add_argument("--data_name", nargs="+", type=str, default="senticap")  # todo: add: "flickrstyle10k"])
...
def initial_variables():
    ...
    desired_labels_list, embedding_vectors_to_load = get_desired_labels(config)
...
def main():
    ...
    gts_per_data_set = get_gts_data(data_path,factual_captions)
        ...
        for label_idx, label in enumerate(desired_labels_list):
            ...
            if config['run_type'] == 'caption':
                ...
                best_caption = run(config, config['img_path'], desired_style_embedding_vector, desired_style_embedding_vector_std,
                    config['cuda_idx_num'], title2print, model_path, config['data_name'],tmp_text_loss,label,img_dict,debug_tracking,text_generator,image_features,text_style_list)
    ...
    for img_name in evaluation_results:
        for label in list(evaluation_results[img_name].keys()):
            if label == 'img_path':
                continue
            evaluation_results[img_name][label]['gt'] = gts_per_data_set[config['data_name']][img_name][label] #todo: handle style type
